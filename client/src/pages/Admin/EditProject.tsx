...
const AddNewProject: React.FC = () => {
  ...
  async function fetchProjectData(id: string) {
    try {
      const res = await projectApi.getSingleProject(id);
      if (res?.project) {
        ...
        setFormData({
          ...
          landArea: project.totalland || "",
          price: project.price || "",
          nearby: project.nearby?.length
            ? project.nearby
            : [{ type: "", name: "", distance: "" }],
        ...
        setExistingImages(project.images || []); // preload server images
      }
    } catch (err) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      let res;
      if (id) {
        res = await projectApi.updateProject(id, form);
        setMessage(res?.message || "Project updated successfully");
      } else {
        res = await projectApi.createProject(form);
      ...
      setMessage(id ? "Error updating project" : "Error creating project");
      setTimeout(() => setMessage(""), 4000);
    }
  };
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div className="text-center mt-4">
            <button type="submit" className="btn btn-success px-4 py-2">
              {id ? "Update Project" : "Submit Project"}
            </button>
          </div>
        </form>
      </div>
